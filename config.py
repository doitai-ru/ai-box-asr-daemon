...
class Settings(BaseSettings):
    ...
    # 0 = декод в основном процессе (фолбэк); N>0 = пул из N процессов
    # (раздельные GIL -> параллель по ядрам). kenlm mmap-шарится между воркерами.
    # Дефолт 6: закрывает ~10 соединений на 10-ядерном бою (под 20 поднять до 8).
    ...
    TONE_FP32: bool = True
    # GPU-профилировщик: СТАРТОВОЕ состояние (по умолчанию выкл — нулевые накладные).
    # На лету тумблится без рестарта: POST /api/v1/admin/gpu-profile {"enabled": true|false}.
    # При вкл: фон-сэмплер NVML + инструментация компонентов -> logs/gpu_profile.jsonl.
    ...
    # Бакеты длины (сек) для паддинга входа GigaAM до фиксированного набора форм:
    # cuDNN/арена греются на эти K размеров, а не на каждую длину чанка (нет спайков
    # латентности и набора видеопамяти под новые формы). На этих же бакетах идёт прогрев.
    # Чанк длиннее макс. бакета режется на куски (аудио не теряется). Пустой список = выкл.
    ASR_PAD_BUCKETS_SEC: list[int] = [4, 8, 16, 30]
