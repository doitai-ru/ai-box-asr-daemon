...
T-one ест ровно TONE_CHUNK_SAMPLES (2400) семплов int32 @ 8 кГц за один forward()
и отдаёт фразы (TextPhrase) с пофразовыми таймингами. Здесь:
  - нарезка входящего PCM16-потока на кадры нужного размера;
  - синтез пословного result'а из фразы (клиент asterisk-socket-server ждёт
    data.result = массив слов со start/end, а T-one даёт только границы фразы).
"""
...
BYTES_PER_FRAME = config.TONE_CHUNK_SAMPLES * 2
...
class StreamResampler:
    ...
    """
    def __init__(self, in_rate: int, out_rate: int = config.TONE_SAMPLE_RATE):
...
def take_frames(buf: bytearray) -> list:
    """
    Достаёт из буфера все полные кадры по TONE_CHUNK_SAMPLES семплов.
...
def flush_tail(buf: bytearray) -> np.ndarray | None:
    ...
    buf.clear()
    pad = config.TONE_CHUNK_SAMPLES - len(samples)
    if pad > 0:
        samples = np.pad(samples, (0, pad))
    return samples[:config.TONE_CHUNK_SAMPLES]
...
def phrase_to_words(text: str, start: float, end: float) -> list:
    ...
    предложений на стороне клиента (words-to-sentences.js) важны относительные паузы -
    при равномерном распределении внутри фразы пауз нет, границы предложений совпадут
    с границами фраз T-one, что и требуется.
    """
    words = text.split()
    if not words:
        return []
...
def phrase_to_data(phrase) -> dict:
    """
    Приводит TextPhrase к формату, который ждёт send_messages / клиент.
